...
class TestRequestCreateServiceTest {
    ...
    public void when_TestRequest_with_valid_data_sent_expect_repository_save_method_called() {
        ...
        Mockito.when(testRequestRepository.save(Mockito.any())).thenReturn(mockedResponse);

        //Act
        // Call createTestRequestFrom

        TestRequest result = testRequestService.createTestRequestFrom(user,createTestRequest);

        //Assert

        Mockito.verify(testRequestRepository).save(any());
        assertNotNull(result);
        assertThat(result.getEmail(),containsString("someone123456789@somedomain.com"));
